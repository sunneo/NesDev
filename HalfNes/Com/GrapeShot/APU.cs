...
namespace HalfNes.Com.GrapeShot
{
    public class APU
    {
        ...
        private List<ExpansionSoundChip> expnSound = new List<ExpansionSoundChip>();
        private bool soundFiltering;
        private static int[] TNDLOOKUP = initTndLookup(), SQUARELOOKUP = initSquareLookup();
        ...
        private bool apuintflag = true, statusdmcint = false, statusframeint = false;
        private int framectr = 0, ctrmode = 4;
        private bool[] lenCtrEnable = { true, true, true, true };
        ...
        private bool dmcsilence = true, dmcirq = false, dmcloop = false, dmcBufferEmpty = true;
        //length ctr instance variables
        private int[] lengthctr = { 0, 0, 0, 0 };
        private static int[] lenctrload = {10, 254, 20, 2, 40, 4, 80, 6,
        160, 8, 60, 10, 14, 12, 26, 14, 12, 16, 24, 18, 48, 20, 96, 22,
        192, 24, 72, 26, 16, 28, 32, 30};
        private bool[] lenctrHalt = { true, true, true, true };
        //linear counter instance vars
        private int linearctr = 0;
        private int linctrreload = 0;
        private bool linctrflag = false;
        ...
        private bool[] envConstVolume = { true, true, true, true };
        private bool[] envelopeStartFlag = { false, false, false, false };
        //instance variables for sweep unit
        private bool[] sweepenable = { false, false },
        ...
        public void setParameters()
        {
            lock (locker)
            {
                ...
                soundFiltering = PrefsSingleton.get().getbool("soundFiltering", true);
                ...
                if (PrefsSingleton.get().getbool("showScope", false))
                {
        ...
        public int read(int addr)
        {
            ...
            switch (addr)
            {
                case 0x15:
                    ...
                    int returnval = ((lengthctr[0] > 0) ? 1 : 0)
                           | ((lengthctr[1] > 0) ? 2 : 0)
                           | ((lengthctr[2] > 0) ? 4 : 0)
                           | ((lengthctr[3] > 0) ? 8 : 0)
        ...
        private static int[][] DUTYLOOKUP = new int[][]{
        ...
        public void updateto(int cpucycle)
        {
            ...
            if (soundFiltering)
            {
                ...
                while (apucycle < cpucycle)
                {
                    ...
                    timers[3].clock();
                    if (expnSound.Count != 0)
                    {
                        foreach (ExpansionSoundChip c in expnSound)
                        {
                            c.clock(1);
                        }
                    }
        ...
        {
            return lpaccum += (int)(0.5 * (sample - lpaccum)); //y = y + a * (x - y)
        }
